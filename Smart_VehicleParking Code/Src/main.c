...
/*Eng : Mohamed Hazem Yahya Mahrous*/
...
void sendStringOverUART(const char *str, USART_t *USARTx) {
    
    for (int i = 0; str[i] != '\0'; i++) {
        
        MCAL_UART_SendData(USARTx, (uint16_t *)&str[i], USART_enable);
    }
}
...
int main(void)
{
	...
	const char *entryGateString = "***** ENTRY GATE *****\r\n***** ENTER RFID CARD *****\r\n";
	sendStringOverUART(entryGateString, USART1);
	...
	const char *exitGateString = "***** EXIT GATE *****\r\n***** ENTER RFID CARD *****\r\n";
	sendStringOverUART(exitGateString, USART2);
...
void Welcome(void)
{
	...
	lcd_set_position(&LCD2,2,0 );
	lcd_send_string(&LCD2,"Hazem Parking");
}
